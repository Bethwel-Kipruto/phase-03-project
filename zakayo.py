...
# engine = create_engine('sqlite:///taxes.db')
...
#     @classmethod
#     def pay_tax(cls, citizen_id, personal_tax_paid, business_tax_paid):
#         citizen = session.query(cls).get(citizen_id)
#         if citizen:
...
#                 taxes_left_to_pay=citizen.tax_details.total_tax - (personal_tax_paid + business_tax_paid)
#             )
...
#         citizens = session.query(cls).filter_by(first_name=first_name, last_name=last_name).all()
#         for citizen in citizens:
#             tax_details = session.query(TaxDetails).filter_by(citizen_id=citizen.id).first()
#             if tax_details:
#                 print(f'Citizen: {citizen.first_name} {citizen.last_name}')
...
# # Paying tax for citizen with ID 1
# Citizen.pay_tax(1, 5000, 2500)
...
# # Finding a citizen's details and tax information using their ID
...
# #Citizen.find_citizens_by_name('Bethwel', 'Kipruto')
...
# engine = create_engine('sqlite:///testinga.db')
# Base = declarative_base()
# Session = sessionmaker(bind=engine)
# session = Session()
...
#     @classmethod
#     def get_citizen_by_id(cls, citizen_id):
#         return session.query(cls).get(citizen_id)
...
# Citizen.pay_tax(citizen_id=1, personal_tax_paid=20000, business_tax_paid=5000)
